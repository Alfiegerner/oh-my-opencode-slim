...
4. Registers into v2 domains:
   ...
   - child-session permission rules → `ctx.session.update({sessionID,
     ...
     (`createPermissionRulesBridge`; v2.0.5 removed `permission.rules`)
...
(v2.0.5-only posture) — a registration failure fails setup loudly. A
